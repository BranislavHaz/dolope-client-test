...
import { getAreaSideWalls } from "@/utils/steps/step4/getAreaSideWalls";
import { getAreaStands } from "@/utils/steps/step4/getAreaStands";
import { getAreaShelfs } from "@/utils/steps/step4/getAreaShelfs";

const Step4 = () => {
  const { state, corpus, sideWalls, productsAPI, stepsInputs, modal } =
    useMainStore((state) => ({
      state: state,
      corpus: state.corpus,
  ...
  const slideSizes = [300, 350, 400, 450, 500, 550, 600, 650];
  ...
  console.log(getAreaDrawers(state));

  /*   const mergeAreas = (state) => {
